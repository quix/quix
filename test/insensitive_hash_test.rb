#
# Based on ruby/branches/ruby_1_8_7/test/ruby/test_hash.rb
...
class TestInsensitiveHash < Test::Unit::TestCase
  ...
  def test_insensitive_hash
    ...
    assert_raises(ArgumentError) {
      InsensitiveHash.new(1, 2)
    }
    assert_raises(ArgumentError) {
      InsensitiveHash.new.fetch(1, 2, 3, 4)
    }

    assert_nil s.default_proc
    assert_not_nil InsensitiveHash.new { }.default_proc
    assert_not_nil s.hash
    assert_equal "a", s.index(33)
    assert_equal "A", u.index(33)
    assert_nil s.index(99)
    
    assert_equal "{:x=>77}", from_hash({:x => 77}).inspect

    assert_equal s, s.merge(u)
    
    assert_equal [["B", 44], ["c", 55]], u.select { |k, v| v > 33 }
  end

  #########################################################
  # remaining tests are for consistency with Hash
  ...
  def test_hash
    ...
    assert_equal(from_hash({1=>2, 2=>4, 3=>6}), x)
    ...
    x = InsensitiveHash.new(0)
    x[1] = 1
    assert_equal(1, x[1])
    assert_equal(0, x[2])

    x = InsensitiveHash.new([])
    assert_equal([], x[22])
